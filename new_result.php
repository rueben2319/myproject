...
<div class="col-lg-12">
  ...
  <div class="col-md-12">
    <div class="d-flex justify-content align-items-center">
      ...
      <div class="form-group col-sm-4">
        <label for="" class="control-label">Select Mark Type</label>
        <select name="" id="mark_type" class="form-control select2 select2-sm input-sm">
          <option value="mid_mark">Mid Term</option>
          <option value="end_mark">End of Term</option>
        </select>
      </div>
      <div class="form-group col-sm-4">
        <label for="" class="control-label" id="mark_label">Mid Term</label>
        <input type="text" class="form-control form-control-sm text-right number" id="mark" maxlength="6">
      </div>
    </div>
    <button class="btn btn-sm btn-primary bg-gradient-primary" type="button" id="add_mark" style="margin-bottom: 5px; margin-left: 10px;" >Add</button>
    <br>
  </div>
</div>
    	<hr>
            ...
            <table class="table table-bordered" id="mark-list">
            	...
            	<tbody>
            		...
            		<tr data-id="<?php echo $row['sid'] ?>">
            			...
            			<td><input type="hidden" name="mid_mark[]" value="<?php echo $row['mid_mark'] ?>"><?php echo $row['mid_mark'] ?></td>
						<td><input type="hidden" name="end_mark[]" value="<?php echo $row['end_mark'] ?>"><?php echo $row['end_mark'] ?></td>
...
<script>
	  // Hide the end-of-term mark input field by default
	  $('#mark_label').hide();
  $('#mark').hide();

  // Show or hide the mark input field based on the selected mark type
  $('#mark_type').on('change', function() {
    if ($(this).val() == 'mid') {
      $('#mark_label').text('Mid Term');
    } else {
      $('#mark_label').text('End of Term');
    }
    $('#mark_label').show();
    $('#mark').show();
  });
